...
\subsection*{Supplement 4 with subjects omitted as suggested by the HCP}
\noindent source and input files available at \texttt{https://osf.io/p6msu/} \par
\noindent compiled \today\  \par
\noindent Supplement 4 for ``Pattern similarity analyses of frontoparietal task coding: Individual variation and genetic influences'' by Joset A. Etzel, Ya'el Courtney, Caitlin E. Carey, Maria Z. Gehred, Arpana Agrawal, and Todd S. Braver. \par
...
quant.RSA.plots <- function(pair.type, which.load) {  # pair.type <- "MZ"; which.load <- "_0and2back";
  ...
  omit.conc <- setdiff(quant.tbl$sub.conc, sub.conc);   # which subject pairs need to be omitted?
  quant.tbl <- quant.tbl[-which(quant.tbl$sub.conc %in% omit.conc),];   # omit the HCP-flagged subject pairs
...
for (sub.lbl in sub.lbls) {   # sub.lbl <- "MZ";
  ...
  sub.conc <- paste0(get.ids(paste0(sub.lbl, ".1s"), hcp.flag=TRUE), ".", get.ids(paste0(sub.lbl, ".2s"), hcp.flag=TRUE)); 
  omit.conc <- setdiff(quant.tbl$sub.conc, sub.conc);   # which subject pairs need to be omitted?
  quant.tbl <- quant.tbl[-which(quant.tbl$sub.conc %in% omit.conc),];   # omit the HCP-flagged subject pairs
...
\vspace{0.2 cm}
\noindent Robust t-tests for difference between Load and Category quantification scores within each subject group. p-values uncorrected for multiple comparisons. (Same as number printed on top row of boxplots.) \par
<<code1i, cache=TRUE, echo=FALSE, results='asis'>>=
...
for (sub.lbl in sub.lbls) {   # sub.lbl <- "MZ";
  ...
  for (comm.lbl in community.ids) {   # comm.lbl <- "Visual";
    stbl <- quant.tbl[which(quant.tbl$comm.id == comm.lbl),]
    y.out <- YuenTTest(diff~type.id, data=stbl, paired=TRUE, alternative="two.sided", trim=do.trim);
    out.tbl[sub.lbl,comm.lbl] <- paste0(get.format(y.out$statistic,2), " (", get.format(y.out$p.value, is.p=TRUE), ")");  
  }
}
print(xtable(out.tbl), include.rownames=TRUE);
...
quant.RSA.plots4 <- function(which.comm, which.quant, which.load) {  # which.comm <- "FrontoParietal"; which.quant <- "load";
  ...
  for (slid in 1:length(sub.lbls)) {   # slid <- 1;
    ...
    sub.conc <- paste0(get.ids(paste0(sub.lbls[slid], ".1s"), hcp.flag=TRUE), ".", get.ids(paste0(sub.lbls[slid], ".2s"), hcp.flag=TRUE)); 
    omit.conc <- setdiff(quant.tbl$sub.conc, sub.conc);   # which subject pairs need to be omitted?
    quant.tbl <- quant.tbl[-which(quant.tbl$sub.conc %in% omit.conc),];   # omit the HCP-flagged subject pairs
...
sub.conc <- paste0(get.ids("MZ.1s", hcp.flag=TRUE), ".", get.ids("MZ.2s", hcp.flag=TRUE));
omit.conc <- setdiff(mz.tbl$sub.conc, sub.conc);   # which subject pairs need to be omitted?
mz.tbl <- mz.tbl[-which(mz.tbl$sub.conc %in% omit.conc),];   # omit the HCP-flagged subject pairs
...
sub.conc <- paste0(get.ids("DZ.1s", hcp.flag=TRUE), ".", get.ids("DZ.2s", hcp.flag=TRUE));
omit.conc <- setdiff(dz.tbl$sub.conc, sub.conc);   # which subject pairs need to be omitted?
dz.tbl <- dz.tbl[-which(dz.tbl$sub.conc %in% omit.conc),];   # omit the HCP-flagged subject pairs
...
sub.conc <- paste0(get.ids("SIB.1s", hcp.flag=TRUE), ".", get.ids("SIB.2s", hcp.flag=TRUE));
omit.conc <- setdiff(sib.tbl$sub.conc, sub.conc);   # which subject pairs need to be omitted?
sib.tbl <- sib.tbl[-which(sib.tbl$sub.conc %in% omit.conc),];   # omit the HCP-flagged subject pairs
...
sub.conc <- paste0(get.ids("UNR.1s", hcp.flag=TRUE), ".", get.ids("UNR.2s", hcp.flag=TRUE));
omit.conc <- setdiff(unr.tbl$sub.conc, sub.conc);   # which subject pairs need to be omitted?
unr.tbl <- unr.tbl[-which(unr.tbl$sub.conc %in% omit.conc),];   # omit the HCP-flagged subject pairs
...
do.dots <- function(sub.1s, do.comm, pair.type) {   # do.comm <- "FrontoParietal"; pair.type <- "MZ"; sub.1s <- MZ.1s;
  ...
  quant.tbl <- read.table(fname.quant, stringsAsFactors=FALSE);
  sub.conc <- paste0(get.ids(paste0(pair.type, ".1s"), hcp.flag=TRUE), ".", get.ids(paste0(pair.type, ".2s"), hcp.flag=TRUE));
  omit.conc <- setdiff(quant.tbl$sub.conc, sub.conc);   # which subject pairs need to be omitted?
  quant.tbl <- quant.tbl[-which(quant.tbl$sub.conc %in% omit.conc),];   # omit the HCP-flagged subject pairs
...
# ALL people without behavioral data are also on the list of people to omit from the HCP (or paired to one), so there aren't any people
# left without behavioral data. several more pairs are omitted (from the HCP list), so the results are still a little different
# than before.
num.MZ.na <- 0; num.DZ.na <- 0; num.SIB.na <- 0; num.UNR.na <- 0;
...
make.behav.plot <- function(do.comm, ttl, which.on.y, load.suffix) {  
  ...
  for (people.lbl in rev(sub.lbls)) {   # people.lbl <- "MZ";    # rev so MZ dots on top
    sub.1s <- get.ids(paste0(people.lbl, ".1s"), hcp.flag=TRUE);
    sub.2s <- get.ids(paste0(people.lbl, ".2s"), hcp.flag=TRUE);
...
get.stats <- function(do.pair, do.comm, which.on.y, load.suffix) {  # do.pair <- "load"; which.on.y <- "pair.mean"; load.suffix <- "0and2back";
  ...
  for (sub.lbl in sub.lbls) {   # sub.lbl <- "MZ"
    sub.1s <- get.ids(paste0(sub.lbl, ".1s"), hcp.flag=TRUE);
    sub.2s <- get.ids(paste0(sub.lbl, ".2s"), hcp.flag=TRUE);
...
single.tbl <- read.table(paste0(in.path, "selfCorrelations/all_selfPairwiseDistanceRSAquant_z_omitSubs.txt"));
...
get.tbl <- function(people.lbl, do.quant, do.comm, load.suffix) {  
  ...
  sub.conc <- paste0(sub.1s, ".", sub.2s); 
  omit.conc <- setdiff(pair.tbl$sub.conc, sub.conc);   # which subject pairs need to be omitted?
  pair.tbl <- pair.tbl[-which(pair.tbl$sub.conc %in% omit.conc),];   # omit the HCP-flagged subject pairs
