...
# if hcp.flag is FALSE, the subject pairings match what was published (Cerebral Cortex, May 2020, https://doi.org/10.1093/cercor/bhz301).
# HOWEVER, the HCP has since suggested that some participants' WM data not be used, see
# https://wiki.humanconnectome.org/display/PublicData/HCP+Data+Release+Updates%3A+Known+Issues+and+Planned+fixes. 20 of these participants were included 
# in the published results. To retrieve pairs of participants omitting the flagged ones, call get.ids() with hcp.flag=TRUE. Then, if one or both
# participant of a pair was flagged by the HCP for omission, both members will be omitted (and so the returned lists of subjects will not include
# any flagged participants).

get.ids <- function(which.group, hcp.flag=FALSE) {
  # this starts with the published subject pairs (some of whom have been marked for omission).
  ...
  ids <- NA;
  if (hcp.flag == TRUE) {    # take out people flagged by the HCP
    omit.ids <- c("bc2101", "bl1733", "cg8943", "cw9972", "eo7543", "ep2070", "fr3903", "gg1348", "ia4019", "ky2646", "oc4214", "ox2573", "qd3657",
                  "sb5631", "uk8158", "vk699", "vr394", "wl1711", "xf5953", "ze7706");  # "Jo id codes for people that need to be omitted
    
    if (which.group == "MZ.1s.GTunique" | which.group == "MZ.1s" | which.group == "MZ.2s.GTunique" | which.group == "MZ.2s") {   # do MZs
      omit.inds <- NA; 
      for (i in 1:length(omit.ids)) {   # i <- 1;
        if (length(which(MZ.1s.GTunique == omit.ids[i])) > 0) { omit.inds <- c(omit.inds, which(MZ.1s.GTunique == omit.ids[i])); }
        if (length(which(MZ.2s.GTunique == omit.ids[i])) > 0) { omit.inds <- c(omit.inds, which(MZ.2s.GTunique == omit.ids[i])); }
      }
      omit.inds <- sort(unique(omit.inds[-which(is.na(omit.inds))]));   # take off starting NA, any duplicates
      
      if (which.group == "MZ.1s.GTunique" | which.group == "MZ.1s") { ids <- MZ.1s.GTunique[-omit.inds]; }
      if (which.group == "MZ.2s.GTunique" | which.group == "MZ.2s") { ids <- MZ.2s.GTunique[-omit.inds]; }
    }
    
    if (which.group == "DZ.1s.GTunique" | which.group == "DZ.1s" | which.group == "DZ.2s.GTunique" | which.group == "DZ.2s") {   # do DZs
      omit.inds <- NA; 
      for (i in 1:length(omit.ids)) {   # i <- 1;
        if (length(which(DZ.1s.GTunique == omit.ids[i])) > 0) { omit.inds <- c(omit.inds, which(DZ.1s.GTunique == omit.ids[i])); }
        if (length(which(DZ.2s.GTunique == omit.ids[i])) > 0) { omit.inds <- c(omit.inds, which(DZ.2s.GTunique == omit.ids[i])); }
      }
      omit.inds <- sort(unique(omit.inds[-which(is.na(omit.inds))]));   # take off starting NA, any duplicates
      
      if (which.group == "DZ.1s.GTunique" | which.group == "DZ.1s") { ids <- DZ.1s.GTunique[-omit.inds]; }
      if (which.group == "DZ.2s.GTunique" | which.group == "DZ.2s") { ids <- DZ.2s.GTunique[-omit.inds]; }
    }
    
    if (which.group == "sib.1s.GTunique" | which.group == "SIB.1s" | which.group == "sib.2s.GTunique" | which.group == "SIB.2s") {   # do SIBs
      omit.inds <- NA; 
      for (i in 1:length(omit.ids)) {   # i <- 1;
        if (length(which(sib.1s.GTunique == omit.ids[i])) > 0) { omit.inds <- c(omit.inds, which(sib.1s.GTunique == omit.ids[i])); }
        if (length(which(sib.2s.GTunique == omit.ids[i])) > 0) { omit.inds <- c(omit.inds, which(sib.2s.GTunique == omit.ids[i])); }
      }
      omit.inds <- sort(unique(omit.inds[-which(is.na(omit.inds))]));   # take off starting NA, any duplicates
      
      if (which.group == "sib.1s.GTunique" | which.group == "SIB.1s") { ids <- sib.1s.GTunique[-omit.inds]; }
      if (which.group == "sib.2s.GTunique" | which.group == "SIB.2s") { ids <- sib.2s.GTunique[-omit.inds]; }
    }
    
    if (which.group == "unr.1s.GTunique" | which.group == "UNR.1s" | which.group == "unr.2s.GTunique" | which.group == "UNR.2s") {   # do UNRS
      omit.inds <- NA; 
      for (i in 1:length(omit.ids)) {   # i <- 1;
        if (length(which(unr.1s.GTunique == omit.ids[i])) > 0) { omit.inds <- c(omit.inds, which(unr.1s.GTunique == omit.ids[i])); }
        if (length(which(unr.2s.GTunique == omit.ids[i])) > 0) { omit.inds <- c(omit.inds, which(unr.2s.GTunique == omit.ids[i])); }
      }
      omit.inds <- sort(unique(omit.inds[-which(is.na(omit.inds))]));   # take off starting NA, any duplicates
      
      if (which.group == "unr.1s.GTunique" | which.group == "UNR.1s") { ids <- unr.1s.GTunique[-omit.inds]; }
      if (which.group == "unr.2s.GTunique" | which.group == "UNR.2s") { ids <- unr.2s.GTunique[-omit.inds]; }
    }
    
  } else {   # use the pairings in the published paper
