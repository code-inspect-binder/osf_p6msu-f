...
\subsection*{Results Supplement 2}
\noindent source and input files available at \texttt{https://osf.io/p6msu/} \par
\noindent compiled \today\  \par
\noindent Results Supplement 2 for ``Pattern similarity analyses of frontoparietal task coding: Individual variation and genetic influences'' by Joset A. Etzel, Ya'el Courtney, Caitlin E. Carey, Maria Z. Gehred, Arpana Agrawal, and Todd S. Braver. \par
\vspace{0.2 cm} 
...
make.RSA.plot <- function(do.comm, pair.type, ttl) {  # do.comm <- "FrontoParietal"; ttl <- ""; pair.type <- "all";
  if (pair.type == "all") { sub.ids <- all.ids; } else { sub.ids <- c(get.ids(paste0(pair.type, ".1s")), get.ids(paste0(pair.type, ".2s"))); }
  
  fname.cor <- paste0(in.path, "selfCorrelations/", pair.type, "_", do.comm, "_selfPairwiseDistanceRSA.txt");
  if (!file.exists(fname.cor)) { 
    self.tbl <- array(NA, c(length(sub.ids), length(pair.ids)));   
    for (sid in 1:length(sub.ids)) {   # sid <- 1;
      fname <- paste0(in.path, "/selfCorr_forRSA/", sub.ids[sid], "_selfPairwiseDistanceRSA_pcor.txt");  # extract from \selfCorrelations\selfCorr_forRSA.zip
...
quant.RSA.plots <- function(pair.type, show.lines=TRUE) {  # pair.type <- "all";
  if (pair.type == "all") { 
    ...
    sub.ids <- c(get.ids(paste0(pair.type, ".1s")), get.ids(paste0(pair.type, ".2s"))); 
    ttl <- pair.type;
  }
  
  # calculate the quantification scores for the self-correlations
  fname.quant <- paste0(in.path, "selfCorrelations/", pair.type, "_selfPairwiseDistanceRSAquant_z.txt");
  if (!file.exists(fname.quant)) {
    ...
    fp.tbl <- read.table(paste0(in.path, "selfCorr_forRSA/", pair.type, "_FrontoParietal_selfPairwiseDistanceRSA.txt"));  
    fp.tbl <- get.FTrz(fp.tbl); # ready for averaging
    v.tbl <- read.table(paste0(in.path, "selfCorr_forRSA/", pair.type, "_Visual_selfPairwiseDistanceRSA.txt"));  # rows are people, columns are pair.ids
...
fname.quant <- paste0(in.path, "selfCorrelations/all_selfPairwiseDistanceRSAquant_z.txt");  # made above
quant.tbl <- read.table(fname.quant, stringsAsFactors=FALSE);
fp.load <- quant.tbl$diff[which(quant.tbl$comm.id == "FrontoParietal" & quant.tbl$type.id == "load")];
fp.st <- quant.tbl$diff[which(quant.tbl$comm.id == "FrontoParietal" & quant.tbl$type.id == "picture")];
v.load <- quant.tbl$diff[which(quant.tbl$comm.id == "Visual" & quant.tbl$type.id == "load")];
...
quant.tbl <- read.table(paste0(in.path, "selfCorrelations/all_selfPairwiseDistanceRSAquant_z.txt"));
...
quant.tbl <- read.table(paste0(in.path, "selfCorrelations/MZ_selfPairwiseDistanceRSAquant_z.txt"));
stbl <- quant.tbl[which(quant.tbl$comm.id == "FrontoParietal"),]
YuenTTest(diff~type.id, data=stbl, paired=TRUE, alternative="two.sided", trim=do.trim);
...
quant.tbl <- read.table(paste0(in.path, "selfCorrelations/DZ_selfPairwiseDistanceRSAquant_z.txt"));
stbl <- quant.tbl[which(quant.tbl$comm.id == "FrontoParietal"),]
YuenTTest(diff~type.id, data=stbl, paired=TRUE, alternative="two.sided", trim=do.trim);
...
quant.tbl <- read.table(paste0(in.path, "selfCorrelations/SIB_selfPairwiseDistanceRSAquant_z.txt"));
stbl <- quant.tbl[which(quant.tbl$comm.id == "FrontoParietal"),]
YuenTTest(diff~type.id, data=stbl, paired=TRUE, alternative="two.sided", trim=do.trim);
...
quant.tbl <- read.table(paste0(in.path, "selfCorrelations/UNR_selfPairwiseDistanceRSAquant_z.txt"));
stbl <- quant.tbl[which(quant.tbl$comm.id == "FrontoParietal"),]
YuenTTest(diff~type.id, data=stbl, paired=TRUE, alternative="two.sided", trim=do.trim);
...
plot.tri.behav <- function(do.pair, do.comm, ttl, xttl, yttl) {  # do.pair <- "allL"; do.comm <- "FrontoParietal"; ttl <- ""; xttl <- ""; yttl <- "";
  ...
  for (sub.lbl in sub.lbls) {   # sub.lbl <- "MZ"
    corr.tbl <- read.table(paste0(in.path, "selfCorrelations/", sub.lbl, "_selfPairwiseDistanceRSAquant_z.txt"));
...
plot.tri.behav <- function(do.pair, do.comm, ttl, xttl, yttl) {  # do.pair <- "allP"; do.comm <- "FrontoParietal"; ttl <- ""; xttl <- "fasdf"; yttl <- "ytttt";
  ...
  mtext(xttl, side=1, cex=0.6, line=1.1);
  mtext(yttl, side=2, cex=0.6, line=0.9);
  corr.tbl <- read.table(paste0(in.path, "selfCorrelations/all_selfPairwiseDistanceRSAquant_z.txt"));
  ...
  # add correlation and p-value; hard-code p-value since all < .001 in table.
  text(x=0.5, y=yLim[1], labels=paste0("r=", doformat(cor(selfs,ds, use='pairwise.complete.obs'),2), " p<.001"), cex=0.7);
  box();
}
...
fp.ts <- c(17, 15.4, 14.6, 15.9);   # sub.lbls order (not ALL)
vis.ts <- c(-28.5, -28.8, -34.8, -28.4);

do.group <- "ALL";     # CHANGE this to "MZ", "DZ", etc. to see other subject grops
if (do.group == "ALL") { 
  use.ids <- all.ids; 
  fp.t <- 31.2; vis.t <- -59.1;   # hard-coded t-test t-values from S2.3 (read off corresponding boxplots)
} else {
  use.ids <- c(get.ids(paste0(do.group, ".1s")), get.ids(paste0(do.group, ".2s")));
...
for (pid in 1:nrow(gordon.key)) {     # pid <- 1; 
  ...
  load.vals <- quant.tbl[use.ids,"to.load"];    # kept z transformed
...
# mean quantification score of Visual and FrontoParietal; vectors made towards the end of code2a
# mean(fp.load, trim=do.trim)  [1] 0.1172779
# mean(fp.st, trim=do.trim)  [1] -0.09609794
# mean(v.load, trim=do.trim)  [1] -0.1412561
# mean(v.st, trim=do.trim)  [1] 0.3018901
...
# print("Parcels with p <= 0.00015 (Bonf. for 333 parcels)");
# stbl <- stat.tbl[which(stat.tbl$p.value <= 0.0001501502),];
# print(stbl[order(stbl$t.value),1:5]);

# nlims <- c(vis.t-2, vis.t);   # more negative than Visual?
# plims <- c(fp.t, fp.t+2);     # more positive than FrontoParietal?
# tmp <- gifti.map(surf.L$data$pointset, surf.L$data$triangle, t.L);
# plot.surface(tmp, paste(do.group, "thresholded t-test ts, L"), pos.lims=plims, neg.lims=nlims);
# tmp <- gifti.map(surf.R$data$pointset, surf.R$data$triangle, t.R);
# plot.surface(tmp, paste(do.group, "thresholded t-test ts, R"), pos.lims=plims, neg.lims=nlims);
